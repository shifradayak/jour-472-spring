...
**Answer**: The warning message says there are one or more parsing issues in the dataset, and to see the specifics of those problems, you should run a line of code that will reveal the problems.  
...
**Answer**: No, it does not. It also sets the vendor zip column as character type rather than dbl, which it was before when R read through less of the dataset to make its determination. 
...
**Answer**: Vendor name is a character data field. Amount is a numerical (dbl) field, and date is a character field. 
...
**Answer**: There are 549 distinct zip codes that do not have five characters, and all of those show up multiple times. I think that people probably entered their zip codes wrong in whatever form or site this data was compiled from, which is why they're showing up like that in the data set. 
...
**Answer**: There's a category other than grants and loans for many of the years — it's presumably made up of N/A items, where the category wasn't entered as a grant or a loan. 
...
**Answer**: The smallest amount was a grant for $60 to the Land Preservation Trust, Inc. The other two smallest amounts are $95 and $186. After that, all the other amounts are greater than $300, so the first couple items on the list seem kind of low.
...
**Answer**: It makes them all lowercase and also separates words with underscores, not spaces 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer**: There are 50 rows that could be duplicates, with 24 distinct grants. 
...
**Answer**: 17,740 rows (as opposed to 17,765 in the non-cleaned dataset)
...
**Answer**: There's a new column that now has standardized zip codes of only five digits each in it. 
...
**Answer**: The zip code 20001 has gotten the most amount of money from the state tourism board, having received $5,094,146,889.61, or $5 billion dollars. This zip code makes sense to me on a numerical level, but I looked it up and it says it's located in central Washington, D.C., which doesn't make sense — why would a D.C. zip code receive grant and loan money from the Maryland tourism board? 

```{r}
cleaned_md_grants_loans |>
  group_by(zip5) |>
  summarise (
    sum_zips_amount = sum(amount)
  ) |> 
  arrange(desc(sum_zips_amount))
```
