...
**Task** Run the following code and describe the dataframe it outputs. **Answer**: The dataframe shows how much money each state spent per student in 2021, how much each state's GDP grew in 2021, how much each state's population changed in 2021, and what presidential candidate won each state in 2020. 
...
**Task** Run the following code. Describe the plot. What pattern do you see? **Answer**: I can see a slightly positive pattern, indicating that more population growth is mildly correlated with greater GDP growth.
...
**Task** Run the following code. Describe the plot. What do you see? **Answer**: There is a very slight negative correlation, but the regression line is mostly horizontal, indicating that there is not a large correlation between states spending more on their students and states experiencing greater GDP growth.
...
**Task** Run the following code. Describe the table output in terms of the strength of relationships between the variables. What do you see? **Answer**: The table shows that there is a weak relationship between per-student spending and GDP growth in 2021. There is a fairly strong negative relationship between population change and per-student spending. Finally,  there is a mildly strong positive relationship between population change and GDP growth. 
...
**Task** Run the following code. Describe the model output. What do you see? **Answer**: The correlation is -0.05, which indicates a weak relationship. The p-value is 0.68, which indicates that there's a high chance that any existing correlation just happened by chance and doesn't actually indicate any real relationship between per-student spending and GDP growth.
...
**Task** Run the following code. Describe the table. What do you see? **Answer**: The table shows contrasting values for states that Biden won versus states that Trump won. Biden states had more GDP growth, spent more on students, and had less population growth than states that Trump won.
...
**Task** Run the following code. Describe the output. What do you see? **Answer**: We can see the averages for population change in Biden states versus Trump states. We can also see a p-value and a t-value. The p-value is pretty small, and the t-value is pretty close to -2, indicating that there is a fairly small chance that these results are by chance. This indicates that there probably is significant correlation between population change and what candidate won each state. 
...
**Task** Run the following code. Describe the output. What do you see? **Answer**: The average per-student spending in states that Biden won is higher than per-student spending states that Trump won. The t-value is well above 2, and the p-value is very low, showing that this difference between states is actually correlated and that it is probably not just by chance.
