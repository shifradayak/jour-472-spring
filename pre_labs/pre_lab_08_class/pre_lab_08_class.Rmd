...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer**: The first line is creating a new dataframe that will save the main dataframe with the new month column without overwriting it. The second line is mutating this dataframe to add a new column for month. To create that column, it's using a function that will pull the month out of the dates that are already provided, and then label them by actual month rather than representing them with numbers. Then, it's lumping the calls together by month, summarizing how many calls occurred in each month, and presenting those so the month with the most calls is at the top and it descends from there.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**: I see a bar chart that represents the number of calls that happened in each month. On the x-axis, the months are arranged by least calls to most calls, so they're out of order. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**: The bars are now horizontal, so the months are on the y-axis and the number of calls are on the x-axis. I think the "coord_flip()" function in the codeblock changed this.
...
**Task** Run the following code. What do you think this does? **Answer**: I think this saves the bar chart as a png file with 16x8 dimensions.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**: I see a table that shows the number of calls on each day in the one-year period from February 6, 2022 to February 6, 2023. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**: I see a line chart with the number of total calls from February 2022 to February 2023. The chart is very convoluted and it's hard to make out specific days within the timeframe represented on the x-axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**: There are now more labels on the x-axis, and they presumably represent weeks within this one-year timeframe. It's still hard to read them, though.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**: I see the same chart as before, but the x-axis labels for each week are now easier to read because they're on more of an angle so they don't overlap.
