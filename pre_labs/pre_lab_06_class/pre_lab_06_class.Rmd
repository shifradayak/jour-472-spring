...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer**: Yes they do, except for the 2022 dataset, where the total column header is written as "Total" and not "TOTAL." I used clean_names to fix this issue.
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer**: There are 24 rows, which is accurate.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer**: It contains population demographic information like the percentage of each county's population in certain racial groups and age groups.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer**: Prince George's County has the highest percentage 18+ voters who are Black, at 60.9% of all its 18+ being Black. 77.09% of voters in Prince George's County are Democrats. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer**: It's a dbl (numeric) column.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer**: ZCTA5N contains data items that are zip codes. 
