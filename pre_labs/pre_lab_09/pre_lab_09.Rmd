...
https://datawrapper.dwcdn.net/ftMer/2/
...
```{r}
read_csv("data/md_gov_county.csv")
...
md_gov_county <- md_gov_county |>
  mutate(moore_pct = moore/(cox+moore+lashar+wallace+harding+write_ins)*100)
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** Describe the changes from the previous map: The map zooms in on just Maryland, since it's only representing data for Maryland FIPS codes. The rest of the country is now hidden and Maryland is more visible.
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}} **Answer** Describe the changes from the previous map: Now, instead of seeing a FIPS code for each county, you can see the actual name of the county. The percentage of Moore votes in each county also displays as a percentage rather than a decimal.
...
https://datawrapper.dwcdn.net/JEmKq/1/
