...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer**: There are 8 columns. Most of them are character or numerical, but one is a data type called multipolygon. 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer**: It contains information about what cities correspond with which zip codes, plus geographical information including latitude and longitude values for each of these cities/zip codes.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: The output is a map of Maryland, presumably by zip code, overlaid on a longitude/latitude grid to represent where on a map each of these zip codes is.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: The code returns a prompt about how to use tigris to pull data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**: The output shows a table similar to the earlier one, but it has codes that correspond to counties rather than zipcodes. It also contains land and water columns, plus the same multipolygon column that shows latitude and longitude values for each county.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: The output is a similar map as before, but with dividing lines that represent counties in Maryland rather than zip codes. They are still overlaid on a longitude/latitude grid.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: This is basically a combination of the last two outputs. The zip codes are layered on top of the county lines, and you can see any overlap (or lack thereof) between each kind of geographical boundary.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: We can see a table that lists the number of foreclosures by zipcode. Some zip codes don't have any foreclosure notices, while others have multiple.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: The output is a table that contains the geographical data on zip codes from earlier, but with an added column that has the number of March 2023 foreclosure notices for each of the corresponding zip codes.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: We can see a heat map that color codes each zip code by the number of foreclsoures in that zip code. Lighter colors represent more foreclosures and darker colors represent fewer foreclosures.
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer**: There wasn't really an output, it just loaded the libraries. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**: We get two tables, one of which lists the number of foreclosure notices by county in Maryland, and the other of which contains a population estimate for each county.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: There's now a geometry table added to the county population table, which contains the longitude and latitude values for each Maryland county.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: This code just displays the tables we're working with.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: We now have a much cleaner table with just the relevant columns for county name, population (which is renamed from the more generic "estimate"), and the geographic latitude and longitude values. The county names in this table also match up with the names in the other table because we took the ", Maryland" out of them.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: We have one table that combines the population and number of foreclosure notices for each county. It still contains the geographic longitude and latitude values for each of those counties.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: We can see a table listing counties with the highest foreclosure rates to the lowest foreclosure rates. The table shows us that foreclosure rates are highest in Charles and Prince George's counties.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: We can see a map with each Maryland county, overlaid on the longitudinal and latitudinal values for each of those counties. The counties are color-coded by the rate of foreclosures they have, with lighter colors representing higher foreclosure rates and darker colors representing lower foreclosure rates.
...
**Task** Run the following code. Describe the output in the space below. **Answer**: The map itself is the same, but this time, it does not contain text labels describing each county, and the color-coding is also different, helping us better distinguish what counties have higher versus lower foreclosure rates. Again, lighter colors mean higher foreclosure rates and darker colors mean lower eviction rates.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer**: The output here is essentially the same as the previous map, but there are fewer colors to represent the different foreclosure rates, because the color-coding is based on a more "sliding" scale since it's looking at the rates logarithmically. 
