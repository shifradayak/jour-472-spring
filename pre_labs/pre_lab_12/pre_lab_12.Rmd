...
**Task** Run the following code and describe the dataframe it outputs. **Answer**: It's a table with four columns, which list the date, title, and URL of each press release, along with the text of each press release. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer**: It condenses the list so that duplicates go away, but differences in capitalization even with the same word still remain.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer**: It's a table of 647,091 words — all of them are lowercase and there are no repeats.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer**: The graphic shows that the most common words in Cardin press releases are the, and, to and similar conjunctions/prepositions/nouns that don't indicate much about the actual content of the releases. Those words are used more than three times as much as some of the more substantial words like "Maryland." The graphic is not that interesting, because the most commonly-occurring words don't really tell us anything definitive.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer**: The result is a dataframe of 50 words, because all the stop words have been removed. This is more interesting than before, because it clues us into the topics of the press releases, since the resulting unique words are more substantial and discuss topics like health, funding, and the like.
...
**Task** Run the following code and describe the results. **Answer**: We get three dataframes of the top 10 terms in press releases for each year. We can see that health is in the top three terms for all three years, but that the percentages of various terms differ from year to year.
...
**Task** Run the following code and describe the results. **Answer**: We see a table of 11 phrases (not sure why it's providing 11 even though the last line of code in this block asked for just the top 10). Some of the top phrases are about COVID-19 and related legislation like the American Rescue Plan Act.
...
**Task** Run the following code and describe the results. **Answer**: There are 10 phrases for 2022, and some of the top results seem to be about funding/projects. Healthcare and COVID-19  are also still prevalent as in the previoys year.
...
**Task** Run the following code and describe the results. **Answer**: Again, there are 10 rows. The top phrases are slightly more different for this year -- health care still shows up, but there are also several phrases about human rights, equality and race.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer**: The results are a dataframe for each of the three years with two columns -- one listing each unique word and the other determining if that word is negative or positive. Most of the sentiments seem correct, but some seem to be interpreted incorrectly. For instance, in 2022, the word "trump" shows up. It's likely that Cardin, a Democrat, talked about Trump in a negative sense, but the function may have interpreted this as "trump" (a synonym of "defeat" rather than a proper noun), and thus assigned it a positive sentiment.
