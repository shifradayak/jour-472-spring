...
**Answer**: Making a new variable is important because we may have to work with all the existing columns at some point, so we don't want to alter our original dataset by getting rid of those columns, which will later cause an error if we try to write a line of code that involves one of those columns. Instead, we create a new variable to store this dataset so we can have both the original version and the version with fewer columns separately.
...
**Answer**: 41011 (which means that there are 41,011 classes that are either in the theatre department or have at least 15 seats)
...
**Answer:** The expenditure_date column is no longer a character column, but instead is a date column. The format of the dates also changed — now, the dates are arranged with dashes rather than slashes, and by year-month-day format rather than month-day-year format.
...
**Answer**: The values are formatted in year-month-day format, but I think that instead of just selecting values that happened on the first day of each of those months, the data is actually presenting everything spent from the first of each month to the last day of each month. I think that this is happening because we created a new month field that lumped together months but still recognized that there are multiple years represented in the dataset.
...
**Answer**: The first line of code is saying we want to alter the general_22 dataset. Then, by writing a mutate function, the code is adding up the number of votes for Cox, Moore, Lashar, Wallace, and any other candidates to create a count of the total number of votes, which goes into its own column. Inside the same mutate function, the code is dividing the number of votes for just Moore by that total number of votes to calculate the percentage of votes Moore got and put them in a new column called pct_moore.
...
**Answer**: Now, the data is ordered by the county where Moore got the largest percentage of the vote to the county where he got the smallest percentage of the vote. 
...
**Answer**: I think that it could be more interesting depending on what you're using the data for. If you're simply writing a story about where Moore garnered the most support, seeing the results arranged in descending order for this column is probably more useful because it gives you a straightforward explanation of what you're looking at. But if you want to write a story about the counties in Maryland that were most divided, or that saw the biggest splits in votes across candidates, seeing this column arranged in ascending order could be helpful.
...
**Answer**: I asked ChatGPT the following question: "What are some functions in R's tidyverse library that I can use to standardize items in a dataset?" Through its answer and additional research on a couple of the functions it listed, I found out that str_trim converts items in a dataset by removing any whitespace, and that str_replace_all will standardize items by finding certain occurrences like phrases in strings and replacing them with whatever you tell R to replace them with. You could use str_replace_all to replace a common abbreviation with a full name so that you can accurately group data rather than having two separate groups of the same thing.
...
**Answer**: I think that the mutate statement is first creating a new conditional column for a new state variable, and then asking R to examine the code to determine if it should put something in that column. Each line within the mutate statement is telling R to look in the address column and see if detects any of the states listed (e.g. Maryland or California). If it does, then the code is saying that R should put an abbreviation for that respective state in the new state column we just created.
...
**Answer**: Yes, it does. It's showing that the largest total expenditure amount came from Maryland (along with the exact amount), and so forth. The N/A category is showing the total amount of expenditures that came from other states not specifically named in this dataset, or from addresses that didn't have a state attached.
...
1.  Whose campaign spent the largest total amount in expenditures in 2021? 

2.  How much in total expenditures came from Maryland/in-state sources in 2021 versus in 2022? 

3.  What spendng category did Jim Rosapepe's campaign spend its highest percentage of expenditures on in 2021 and 2022?
