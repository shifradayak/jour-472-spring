...
**Answer: The first line is going into the dataframe called umd_courses. With the symbol, it's then telling R to group the pieces of data in the dataframe by the term column. It calls the symbol again, to tell R to then summarize  its findings of that grouping by providing a count of the number of classes, represented by n, in each term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: There's an additional command in this codeblock, which directs R to arrange the count in descending order. As a result, the counts are the same and the data is grouped the same way, but it's now in descending order of terms with the most classes offered to least classes offered.**
...
**Answer: The output now tells you how many classes are offered in each separate department per term, rather than just giving you total number of classes offered per term. Changing the columns we wanted to group by in the code did this — rather than just the one term column, we said we wanted to arrange by term but also by department. **
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: This codeblock  adds something under the summarise function — in addition to a line of code asking R to provide number of classes when grouped by term, there's also a line asking R to sum all the seats in those classes and provide that result too. As a result, rather than the output giving us number of classes offered per department per term, we now can see the total number of classes offered per term, as well as the total number of seats across all those classes per term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: This codeblock has more summarizations than the previous one. In addition to the line of code asking for a count of total classes and total seats across those classes offered per term, there are also lines of code asking R to calculate the mean number of seats and median number of seats offered in a class per term. As a result, the output has two additional columns which show mean and median seats for each term row. Both top terms in this codeblock and the previous codeblock are the ones with the highest number of total seats available. **
...
**Answer: The group by column is different in this codeblock, which results in the data being presented by departments rather than by terms in the output. There are also two additional lines of code that ask R to summarize the data by providing a minimum and maximum number of seats offered in classes in each department, which reflects in the result by adding two columns representing that information. The data is sorted by department, and arranged in descending order from the department that offers the largest total number of seats in its classes to the smallest total number of seats.**
...
1.  Are there certain disciplines (for instance, STEM classes versus humanities classes) that have larger average class sizes than others?

2.  What is the engineering cooperative education department and why are the maximum and average number of seats offered in classes in that department so large compared to other classes at the university? 

3.  Of the different language departments, which have more versus less offerings and more versus less seats at this university, and what does that reflect about the university's resources to teach different languages?
