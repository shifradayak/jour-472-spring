...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with Y. Name a problem that you see that is preventing proper grouping. 
**Answer**: There are a lot of grantee names that are the same, but displayed differently (e.g. Y OF CENTRAL MARYLAND vs. Y Of Central Maryland). These are not being grouped together because R is reading them as different values. 
...
Run the following code to make a copy of the Grantee field called grantee_clean. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. 
**Answer**: The refinr package changed the case of some of the grantees — for instance, it turned Washington County Museum of Fine Arts into WASHINGTON COUNTY MUSEUM OF FINE ARTS. It also standardized some grantees by removing "Inc" from their names.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with Y. Name three changes the refinr function made in the first 30 results. 
**Answer**: The refinr function changed a value of YMCA OF FREDERICK COUNTY MD INC to YMCA OF FREDERICK COUNTY MD. It also changed two different values representing the YMCA of Central Maryland to match a third value, so all three became YMCA OF CENTRAL MARYLAND. It also removed an "Inc" from Yeshiva of Greater Washington, standardizing those values into "YESHIVA OF GREATER WASHINGTON." 
...
Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with Y. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? 
**Answer**: No, the changes don't vary that much. 
