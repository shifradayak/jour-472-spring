...
```{r echo=FALSE}
census_api_key("ef5457f8a2b11d9eecec191d6867f76f96db192b", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer**: Nevada had the highest percent change in population between 2000 and 2010. 
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer**: The headline could be "Howard County has the highest median household income in Maryland."
...
Is the difference between Calvert and Montgomery significant? **Answer**: No, it's not, because the bars overlap.

Is the difference between Howard and everyone else significant? **Answer**: Yes, because the bars for Howard do not overlap with any other counties.
