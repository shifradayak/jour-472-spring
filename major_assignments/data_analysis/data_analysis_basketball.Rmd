...
```
Link to final memo: https://docs.google.com/document/d/162Q4uKlZH7Eu760_q2sSBVzzL0kzal0ii7GMLGZFeiM/edit?usp=sharing 
...
#We know from our exploratory analysis that the 2020-21 season had 13075 players total signed, the 2021-22 roster had 13645 players total signed, the 2022-23 roster had 13803 players total signed, and the 2023-24 roster had 13907 players total signed. To show the number of international students as a percentage of the total players per season, we're going to mutate using these numbers 
...
```
```{r}
#We're also curious about what division most international players go to, so we can calculate the number of international students in each division with the following code

all_rosters |>
filter(country_clean != "USA") |>
  group_by(division) |>
  summarise(
    international_division = n()
  ) 

```

Answer: The share of international students that make up each season's NCAA women's basketball roster has grown fairly linearly from the 2020-21 season to the 2023-24 season, indicating an increase in the percentage of international students on each season's roster. Also, we can see that most international students who are signed to NCAA teams have gone to Division I over the past four seasons. 
...
```
```{r}
#Out of curiosity, let's also group by division to see what division sees the most redshirts
all_rosters |>
  filter (redshirt == 1) |>
  group_by (division) |>
  summarise (
    redshirt_count_by_division = n()
  ) |>
  arrange(desc(redshirt_count_by_division))


```


Answer: In the 2023-24 year, players were most likely to redshirt during their sophomore year. Junior year followed, and then freshman year. Senior year and a player's fifth year were the least common. In terms of the most and least popular options, about 31% more players redshirted during their sophomore year than their fifth year. Looking at all seasons in total, the most redshirts happened in divisions I and II. Division III, by comparison, sees more than 270 times fewer redshirts than Division I.
